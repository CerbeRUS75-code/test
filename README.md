...
![image](https://github.com/user-attachments/assets/3775272b-f0c2-4570-a18a-8c296a6b92cb)
...
__Как обеспечивается безопасность в http запросах?__
...
![image](https://github.com/user-attachments/assets/4cd63fed-026b-42be-99a9-83cd17b236d4)
...
__- Map и concurrency
    - Что произойдет при конкуррентной записи в мапу?
    - Как работает эвакуация данных?
    - Потокобезопасная ли мапа?
    - Map vs Sync.Map__

1. Что произойдет при конкуррентной записи в мапу?
В Go мапа не является потокобезопасной. Если несколько горутин одновременно пытаются читать или записывать данные в мапу, это приведет к состоянию гонки (race condition). В лучшем случае это вызовет непредсказуемое поведение, а в худшем — панику программы с ошибкой fatal error: concurrent map read and map write.
3. Потокобезопасная ли мапа?
Нет, мапа в Go не является потокобезопасной. Если несколько горутин одновременно работают с мапой (читают или записывают), это приведет к состоянию гонки. Чтобы избежать этого, необходимо использовать механизмы синхронизации, такие как sync.Mutex, sync.RWMutex или sync.Map.
4. Map vs Sync.Map

Обычная мапа (map)
Плюсы:

Высокая производительность для однопоточных или синхронизированных сценариев.
...
Требует ручной синхронизации (например, с использованием sync.Mutex).
...
Оптимизирована для сценариев, где ключи часто читаются, но редко записываются.

Не требует ручной синхронизации.

Минусы:

Менее производительна, чем обычная мапа, в однопоточных сценариях.

Более сложный API (например, методы Load, Store, Delete, Range).

__Какая сложность работы с мапой?__

![image](https://github.com/user-attachments/assets/ce6b4203-5a78-457a-a8b1-49ccf7e3349c)

__1. Чем массив отличается от слайса? Чем хорош массив по сравнению со слайсом?
2. Как работает append?
3. Какие методы оптимизации работы со слайсами ты бы применил в работе?
4. Что такое слайс?
5. Какая есть функции для создания слайса с длиной отличной от нуля?
6. С какой скоростью идет поиск в массиве и почему?__

2) Внутренняя работа:

Проверяется, достаточно ли емкости слайса для добавления новых элементов.

Если емкости недостаточно:

Создается новый массив с увеличенной емкостью.
...
a) Предварительное выделение емкости
Если вы знаете заранее, сколько элементов будет в слайсе, выделите память с помощью make, указав длину и емкость. Это уменьшит количество переаллокаций памяти.

b) Использование copy вместо append
Если вам нужно скопировать данные из одного слайса в другой, используйте copy. Это быстрее, чем создавать новый слайс с помощью append.

5)

Чтобы создать слайс с ненулевой длиной, используем функцию make. Она позволяет указать длину и емкость слайса.

6)

Поиск в массиве (или слайсе) имеет временную сложность O(n), где n — количество элементов в массиве. Это связано с тем, что массив — это последовательная область памяти, и для поиска элемента нужно пройтись по всем элементам, пока не будет найден искомый.
